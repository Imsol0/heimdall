...
func createLogClient(key []byte, url string) (*client.LogClient, error) {
	...
	opts := jsonclient.Options{PublicKey: string(pemPK), UserAgent: "heimdall-" + uuid.New().String()}
...
func PopulateLogs(logListURL string) ([]types.CtLog, error) {
	...
	for _, operator := range usable.Operators {
		for _, log := range operator.Logs {
			// Skip bogus/placeholder logs
			if strings.Contains(strings.ToLower(log.Description), "bogus") ||
				strings.Contains(strings.ToLower(log.Description), "placeholder") ||
				strings.Contains(strings.ToLower(log.Description), "example.com") ||
				strings.Contains(strings.ToLower(log.URL), "example.com") {
				continue
			}
